...
func Book(c *gin.Context) {
	...
	if bookPath == "" || locale == "" {
		common.SendErrorResponse(c, http.StatusNotFound, clientTime, "参数不全")
	}
	...
	if err != nil {
		common.SendErrorResponse(c, http.StatusInternalServerError, clientTime, "Database query error")
		return
	}

	if len(chapters) == 0 {
		common.SendErrorResponse(c, http.StatusNotFound, clientTime, "Chapter does not exist or no matching documents found")
		return
	}
	...
	common.SendSuccessResponse(c, clientTime, bookDatas)
}
...
func BookStatistic(c *gin.Context) {
	...
	if category == "" || book == "" {
		common.SendErrorResponse(c, http.StatusBadRequest, clientTime, "Book Don't Exist")
		return
	}
	...
	if err != nil {
		common.SendErrorResponse(c, http.StatusInternalServerError, clientTime, "Error fetching book statistics")
		return
	}
	...
	common.SendSuccessResponse(c, clientTime,
		bookStatistic{
...
func BookMeta(c *gin.Context) {
	...
	if filePath == "" {
		common.SendErrorResponse(c, http.StatusInternalServerError, clientTime, "Failed Find Target Category")
		return
	}
	...
	if err != nil {
		common.SendErrorResponse(c, http.StatusInternalServerError, clientTime, "Failed load data")
		return
	}
	common.SendSuccessResponse(c, clientTime, data)
}
...
func UpdateBookMeta(c *gin.Context) {
	...
	if err := json.Unmarshal([]byte(metas_data), &metas); err != nil {
		common.SendErrorResponse(c, http.StatusBadRequest, clientTime, err.Error())
		return
	}
	...
	if err != nil {
		common.SendErrorResponse(c, http.StatusInternalServerError, clientTime, "Failed Save Data")
		return
	}
	...
	common.SendSuccessResponse(c, clientTime, "Data saved successfully")
